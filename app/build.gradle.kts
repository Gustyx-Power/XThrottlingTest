...
// Task to rename/copy app-release.apk to XThrottlingTest-versionName.apk after assembleRelease
...
val renameReleaseApk by tasks.registering(Copy::class) {
    ...
    // Mengganti nama file
    rename { "XThrottlingTest-$versionName.apk" }
}
...
val uploadReleaseApkToTelegram by tasks.registering(UploadApkToTelegramTask::class) {
    ...
    val versionName = android.defaultConfig.versionName ?: "unknown-version"
    val outputApkPath = layout.projectDirectory.file("dist/XThrottlingTest-$versionName.apk")
